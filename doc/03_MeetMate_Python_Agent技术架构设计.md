...
> - **v1.2 修订（基于二次评审）**：澄清契约收敛为 MVP 方案 A（仅 `USER`/`OWNER`，群体澄清由 Java 为每位成员各建一条），补充 `request_clarification` 内部接口返回 `clarificationId`；方案返回 `candidate_id` 而非 `shop_id`；新增 ResumeDispatcher 可靠投递（Java 侧）；修正 `wait_for_confirmation` 统一走澄清链路；明确 Score 的「输入允许 / 输出禁止」边界；Checkpoint 配置固定为 Redis。
...
flowchart TD
    ...
    K --> K2[request_clarification]
    K2 --> K3[interrupt_waiting_input]
...
> **v1.2 修正**：原 `propose_relaxation → wait_for_confirmation` 分支改为 `propose_relaxation → request_clarification → interrupt_waiting_input`，与信息缺失分支统一走 R1 澄清链路（Java 落 `tb_meet_clarification` 并切 `WAITING_INPUT`，用户经 Java 作答后恢复）。不再保留无契约定义的 `wait_for_confirmation` 节点（ADR-002）。
...
class ProposalSuggestion(BaseModel):
    proposal_no: int
    candidate_id: str          # 对应 Java Ranking Snapshot 候选；Java 据其解析 shopId + suggestedTime
    positioning: str
    explanation: str
...
发现缺失后调用 Java Tool `request_clarification` 让 Java 落 `tb_meet_clarification` 并切 `AgentRun = WAITING_INPUT`，Graph 通过 LangGraph interrupt 挂起；用户作答后由 Java 调 `resume`，Python 用 `Command(resume=...)` 恢复（见 §5.3、§17）。Python 不持有澄清答案，也不直接通知前端。Java 返回 `clarificationId`，Python 保存该 id 作为后续 resume 的幂等键（见 §12、Java 文档 §18.3）。
...
class JavaToolClient:
    ...
    async def request_clarification(self, req: ClarificationRequest) -> ClarificationCreated:
        # Java 返回 { clarificationId, status }，Python 保存 clarificationId 用于 resume 幂等
        ...
...
**Schema 边界（输入 vs 输出）**：Python **输出** Schema（`ProposalSuggestion` 等）严禁任何 `score` 字段（ADR-001）；但对 **Java 传入的输入数据**（Ranking Snapshot 中的 `RankedCandidate`、resume answer）必须显式声明并**允许**权威评分字段（`groupScore` / `fairnessScore` / `memberScores` 等）——Python 需要这些分数来选择候选并生成解释。因此"拒绝未知字段"仅针对 **输出方向**；输入方向的候选评分快照按 `RankedCandidate` Schema 正常接收，不得误拒合法 score 字段。resume answer 仍按 `ClarificationAnswer` Schema 校验。
...
- 不推荐已否决店铺；
- 不引用不存在字段；
- 方案 `candidate_id` 必须存在且命中 Ranking Snapshot 候选；`shop_id` / `suggested_time` 由 Java 从快照解析，Python 不直接填。
...
AGENT_CHECKPOINT_REDIS_URL=redis://agent-user:***@redis:6379/0
AGENT_CHECKPOINT_PREFIX=meetmate:agent:checkpoint
AGENT_CHECKPOINT_RUNNING_TTL_SECONDS=86400
AGENT_CHECKPOINT_FINISHED_TTL_SECONDS=604800
LLM_PROVIDER=***
LLM_MODEL=***
LLM_API_KEY=***
...
22. 严格 Schema：输出禁 score、输入允许权威 score（R2，见 §14 边界说明）；
23. 实现 `propose_relaxation` 走 `request_clarification` 澄清链路（§7，R1）；
24. `request_clarification` 接收并返回 `clarificationId` 用于 resume 幂等（§12）；
25. Checkpoint 明确 Redis 后端配置（§28）。
