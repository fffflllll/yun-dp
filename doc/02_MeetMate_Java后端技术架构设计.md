...
> **v1.2 修订（基于二次评审）**：补齐编码前仍缺失的实体与跨服务可靠性契约：
> - 澄清契约收敛为 MVP 方案 A：`tb_meet_clarification` 仅支持 `USER` / `OWNER`，群体澄清由 Java 为每位成员各建一条；补充 `request_clarification` 内部接口（`POST /internal/agent-tools/runs/{runId}/clarifications`）与可靠 resume 投递（ResumeDispatcher + `resume_status`）；
> - 新增 `tb_meet_round` 轮次实体，冻结有效投票人 / 多数票门槛 / 投票截止 / Ranking Snapshot / 选中方案，使多轮重规划与超时补偿有持久化依据；
> - 方案绑定候选：`tb_meet_proposal` 增加 `run_id` / `ranking_snapshot_id` / `candidate_id`，Python 返回 `candidate_id` 而非 `shop_id`（同店多时间需区分）；
> - 修正状态机冲突：`VOTING → EXPIRED` 删除，补充 `REPLAN_PENDING → FINALIZED` 与 `REPLANNING → FINALIZED`；`startPlanning` 改用 `ensureRoomReadyToPlan`；Agent 超时按 `runType` 补偿房间状态（`status=FAILED, errorCode=RUN_TIMED_OUT`）；活跃 Run 限制覆盖 `LOCAL_REPLAN`；
> - `tb_shop_meet_meta` 完善来源枚举与 UNKNOWN 按字段处理策略，新增种子数据脚本；Checkpoint 配置明确为 Redis。
...
├── candidateId
├── proposalNo
├── shopId            # 由 candidateId 解析得到（快照副本，便于展示）
├── suggestedTime     # 由 candidateId 解析得到（快照副本）
├── estimatedPerCapita
├── groupScore
├── fairnessScore
├── explanation
...
stateDiagram-v2
    ...
    REPLAN_PENDING --> FINALIZED
    REPLANNING --> VOTING
    REPLANNING --> FINALIZED
...
> 恢复边（v1.2 补充）：局部重规划**失败或房主取消**时回到 `FINALIZED`——`REPLAN_PENDING → FINALIZED`（房主放弃重规划）与 `REPLANNING → FINALIZED`（`LOCAL_REPLAN` 超时，旧方案继续有效）；投票截止无方案过半只进 `REPLAN_PENDING`，不再直接 `EXPIRED`（见 §32.4）。
...
同一房间同一时刻只能有一个活跃规划任务，无论 `runType` 是 `PLAN`、`REPLAN` 还是 `LOCAL_REPLAN`：
...
处于活跃状态（约束由 `roomId + activeRunType` 唯一键 + Redisson 锁保证，见 §20.2）。
...
CREATE TABLE tb_meet_proposal (
    ...
    ranking_snapshot_id VARCHAR(64) NOT NULL,
    ...
    KEY idx_snapshot_candidate(ranking_snapshot_id, candidate_id)
);
```
...
### 10.8.1 数据来源治理（UNKNOWN 策略）

`source` 枚举：

- `MANUAL_SEED`：人工构造测试数据；
- `ADMIN_VERIFIED`：管理端维护并审核；
- `MERCHANT_PROVIDED`：商户自行提供；
- `USER_REPORTED`：用户上报；
- `EXTERNAL_PROVIDER`：外部数据供应商；
- `LLM_INFERRED`：模型推断（**仅可用于软推荐，禁止参与任何硬约束判断**）。

MVP 允许参与硬约束的来源仅限 `MANUAL_SEED` / `ADMIN_VERIFIED`；其余来源仅用于软偏好加权。

种子数据脚本位置：`src/main/resources/db/meetmate_shop_meta_seed.sql`，首期至少准备 30～50 家结构完整的演示店铺。

硬约束字段缺失（值 UNKNOWN）时的处理按字段分级，不得一律“跳过该项硬约束”：

| 字段 | 约束类型 | UNKNOWN 处理 |
|---|---|---|
| 过敏原 | HARD | 淘汰或标记不可验证，不得宣称安全 |
| 辣度（明确不吃辣） | HARD | 辣度未知则淘汰 |
| 营业时间 | HARD | 无法确认则淘汰 |
| 距离 | HARD | 经纬度缺失则淘汰 |
| 地铁距离 | SOFT | 不加分、不淘汰 |
| 环境氛围 | SOFT | 不加分 |
| 排队时间 | SOFT | 标记未知、展示但不伪造 |
...
CREATE TABLE tb_meet_clarification (
    ...
    -- 可靠 resume 投递状态（见 §33.6 ResumeDispatcher）
    resume_status VARCHAR(16) NOT NULL DEFAULT 'NOT_READY',
    resume_attempts INT NOT NULL DEFAULT 0,
    next_retry_at DATETIME NULL,
    resume_error VARCHAR(512) NULL,
    ...
    KEY idx_resume_pending(run_id, resume_status)
);
```

> **MVP 方案 A（澄清多回答模型收敛）**：`target_type` 仅支持 `USER` / `OWNER`，**不支持 `ALL_MEMBERS` / `ANY_MEMBER`**，因此一张 clarification 只保存一位回答人的答案。需要“群体澄清”时，由 Java 为每位目标成员**分别创建一条 clarification**（各自 `target_user_id`、`status=PENDING`），再按各自作答驱动 resume。相应 `resume_policy` 仅保留 `IMMEDIATE` / `OWNER_CONFIRMATION`，删除 `ALL_REQUIRED_ANSWERED`（多回答子表 `tb_meet_clarification_answer` 留待后续版本）。
...
- `resume_status`：`NOT_READY` / `PENDING` / `DISPATCHING` / `ACKED` / `FAILED`
- `resume_policy`：`IMMEDIATE` / `OWNER_CONFIRMATION`
...
> 支撑「轮次实体」——进入 `VOTING` 时冻结有效投票人、多数票门槛、投票截止、Ranking Snapshot 与选中方案，使多轮重规划与 Agent 超时补偿有可持久化依据（R3 / 投票规则 ADR-004）。

```sql
CREATE TABLE tb_meet_round (
    id BIGINT PRIMARY KEY,
    room_id BIGINT NOT NULL,
    round_no INT NOT NULL,
    round_type VARCHAR(32) NOT NULL,
    status VARCHAR(32) NOT NULL,
    eligible_voter_ids_json JSON NOT NULL,
    eligible_voter_count INT NOT NULL,
    majority_count INT NOT NULL,
    voting_started_at DATETIME NULL,
    voting_deadline DATETIME NULL,
    ranking_snapshot_id VARCHAR(64) NULL,
    selected_proposal_id BIGINT NULL,
    create_time DATETIME NOT NULL,
    update_time DATETIME NOT NULL,
...
POST /internal/agent-tools/runs/{runId}/clarifications
POST /internal/agent-tools/runs/{runId}/progress
POST /internal/agent-tools/runs/{runId}/proposals
```

## 18.3 澄清创建接口（R1，对应 §33）

Python 发现信息缺失时调用，由 Java 落 `tb_meet_clarification` 并切 `AgentRun = WAITING_INPUT`：
...
```json
{
  ...
  "expirePolicy": "FAIL_RUN",
  "expiresAt": "2026-07-12T18:00:00+08:00"
}
```

响应（**必须返回 Java 实际生成的 clarificationId**，否则 Python 无法建立 `clarificationId → resume` 对应）：
...
Java 校验：`runId` 属于 `roomId`、`run` 处于 `RUNNING`、同一 `questionCode` 未重复挂起。群体澄清时 Java 为每位目标成员各建一条（MVP 方案 A，见 §10.9）。
...
> **部署同步（R9）**：`docker-compose.yml` 的 build context 必须指向 `web/`（前端新工程），原 `frontend/` 已重命名为 `frontend-legacy/` 并保留为只读历史；nginx 的 `/api` 反代目标为 `java-app:8081`（与 IDEA 直连的 8081 端口一致）。SSE 流需关闭 Nginx 缓冲（见前端文档 §20）。
---
...
26. 实现 `tb_meet_round` 轮次实体与有效投票人冻结（投票规则、R3）；
27. 实现 `request_clarification` 内部接口（§18.3）与 clarification 多回答收敛（MVP 方案 A）；
28. 实现 ResumeDispatcher 可靠投递（§33.6，`resume_status` 状态机）；
29. 实现 Agent 超时按 `runType` 补偿房间状态（§32.6，`errorCode=RUN_TIMED_OUT`）；
30. 落地 `tb_shop_meet_meta` 种子数据与 UNKNOWN 按字段处理（§10.8.1）。
...
- 投票轮次冻结于 `tb_meet_round`（有效投票人 / 多数票门槛 / 投票截止 / Ranking Snapshot / 选中方案），重规划第二轮有明确依据；
- clarification 作答后以 ResumeDispatcher 可靠投递至 Python，仅 `ACKED` 才算恢复成功，网络超时可重试不丢消息。
...
- `startPlanning` → `ensureRoomReadyToPlan(room)` + `ensureNoActivePlanningRun(room)`
...
- `finalize` / 房主确认重规划 → `ensureRoomInStatus(room, 允许状态)`

> 注意：`startPlanning` 在偏好截止到达后**仍应允许**——此时房间已因偏好截止扫描转为 `READY_TO_PLAN`。因此 `startPlanning` 使用 `ensureRoomReadyToPlan`（房间处于 `READY_TO_PLAN` 且当前无活跃规划 Run），而非 `ensurePreferenceOpen`（偏好收集已关闭）。`ensurePreferenceOpen` 仅用于偏好收集阶段（提交/修改偏好、邀请成员）。
...
- `RUNNING` 且 `heartbeatAt` 超过阈值（或 `runDeadline` 到达）→ `status = FAILED`，`errorCode = RUN_TIMED_OUT`（**不引入独立 `TIMED_OUT` 状态**，保持 AgentRun 状态机简单）；
- 否则 Python 进程崩溃后房间可能永久停留 `PLANNING` / `REPLANNING`；
- 房间侧按 `runType` 补偿（扫描在更新 run 后触发，幂等）：
  - `PLAN` 超时：`PLANNING → READY_TO_PLAN`（可重试规划）；
  - `REPLAN` 超时：`REPLANNING → REPLAN_PENDING`（等待房主确认是否再重规划）；
  - `LOCAL_REPLAN` 超时：`REPLANNING → FINALIZED`（旧最终方案继续有效，见 §8 恢复边）。
...
## 33.6 可靠 resume 投递（ResumeDispatcher）

仅用「事务提交后同步调 Python」存在投递缺口：答案已落库（`clarification=ANSWERED`）但调 Python `resume` 时网络超时，会导致 **clarification 已答、AgentRun 仍 `WAITING_INPUT`、Python 未恢复**。幂等键只防重复，不保证一定送达。

`tb_meet_clarification` 增加 `resume_status` / `resume_attempts` / `next_retry_at` / `resume_error`（见 §10.9），流程：

1. 用户提交答案事务内：`answer_json` 写入、`status=ANSWERED`、`resume_status=PENDING`；
2. 后台 `ResumeDispatcher`（`@Scheduled`，Redisson 锁，分页扫描 `resume_status=PENDING`）调用 Python `resume`；
3. Python `ACK` 后 → `resume_status=ACKED`；失败 → `resume_attempts+1`、`next_retry_at=now+backoff`，超过上限置 `FAILED` 并告警（可人工干预或转 `WAIT_OWNER_DECISION`）；
4. Python 侧按 `clarificationId` 去重，重复 resume 返回原状态，不重复执行节点。

> MVP 采用数据库扫描器即可，不引入 Outbox 框架；后续若需更强实时性可升级为 `meet.agent.resume.requested` 领域事件 + 消费者。
...
class ProposalSuggestion(BaseModel):
    proposal_no: int
    candidate_id: str          # 对应 Java Ranking Snapshot 中的候选，由 Java 解析出 shopId + suggestedTime
    positioning: str
    explanation: str
...
```json
{
  ...
  "candidates": [
    {
      "shopId      {
        "candidateId": "cand_01J...", "shopId": 10, "suggestedTime": "2026-07-18T19:00:00+08:00", "rank": 1, "groupScore": 86.2, "fairnessScore": 91.0,
      "memberScores": [
...
Java 根据 `rankingSnapshotId + candidateId` 重新取得权威评分与快照字段（`shopId` / `suggestedTime` / 各项分数）后写入 `group_score` / `fairness_score` / `member_score_json` / `estimated_per_capita` / `suggested_time`。同店可能存在多个候选时间，故以 `candidateId` 而非仅 `shopId` 定位（见 §10.10、§10.5）。

> **Schema 边界（输入 vs 输出）**：Python **输出** Schema（`ProposalSuggestion`）严禁任何 `score` 字段；但 Java 传给 Python 的 **输入** Schema（`RankedCandidate`，即 Ranking Snapshot 中的候选）必须显式声明并允许权威评分字段（`groupScore` / `fairnessScore` / `memberScores` 等）——Python 需要这些分数来选择候选并生成解释。两者方向相反，不得混为一谈：拒绝「Python 回传 score」，但**接受**「Java 传入 score 供解释引用」。
